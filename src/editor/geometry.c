...
void geometry_cube_create_vertices(float *box) {
  array_init(&selected_model->vertex_array, sizeof(PVertex), 8);

  struct Vertex back_right_down; // min
  memset(&back_right_down, 0, sizeof(PVertex));
  struct Vertex front_left_up; // max
  memset(&front_left_up, 0, sizeof(PVertex));
  ...
  glm_vec3_copy((vec3){back_right_down.position[0], front_left_up.position[1],
                       back_right_down.position[2]},
                front_right_down.position);

  glm_vec3_copy((vec3){front_left_up.position[0], back_right_down.position[1],
                       front_left_up.position[2]},
                back_left_up.position);

  glm_vec3_copy((vec3){front_left_up.position[0], front_left_up.position[1],
                       back_right_down.position[2]},
                front_left_down.position);

  glm_vec3_copy((vec3){back_right_down.position[0], back_right_down.position[1],
                       front_left_up.position[2]},
                back_right_up.position);

  glm_vec3_copy((vec3){front_left_up.position[0], back_right_down.position[1],
                       back_right_down.position[2]},
                back_left_down.position);

  glm_vec3_copy((vec3){back_right_down.position[0], front_left_up.position[1],
                       front_left_up.position[2]},
                front_right_up.position);
...
void geometry_sphere_create_vertices(int vertice_count) {
  array_init(&selected_model->vertex_array, sizeof(PVertex), 360);
  ...
  float Y;
  float Theta;
  float WedgeAngle; // Size of angle between two points on the circle (single
                    // wedge)
